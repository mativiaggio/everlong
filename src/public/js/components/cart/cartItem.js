...
export function cartItem(product) {
  return `
    <div id="product-card-${
      product._id
    }"  class="mb-2 border border-[var(--main-light-2)] dark:border-[var(--main-dark-8)] rounded-lg overflow-hidden shadow relative bg-[var(--main-bg-light)] dark:bg-[var(--main-dark-3)]">
      ...
      <div class="p-6 w-full flex flex-col sm:flex-row justify-between">
        <div class="flex">
          ...
          <div class="flex flex-col justify-between px-2">
            ...
            <div class="border w-fit">
              <button class="removeOne px-2" data-button="${
                product._id
              }">-</button>
              <span id="quantity-${product._id}">${product.quantity}</span>
              <button class="addOne px-2" data-button="${
                product._id
              }">+</button>
            </div>
          </div>
        ...
        <div class="flex flex-row sm:flex-col mt-2 sm:mt-0 items-center">
          ...
          <p class="text-sm text-center text-[var(--main-light-5)]">Stock: ${
            product.stock
          }</p>
        </div>
      </div>

      <button class="deleteButton absolute top-2 right-2" data-button="${
        product._id
      }">
